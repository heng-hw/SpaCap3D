...
-------------------
Add function savetxt: save each scene as .txt file [x,y,z,sem_id,inst_id]
Usage example:
export_one_scan('scene0000_00', './scans/scene0000_00/scene0000_00', savetxt=True, savenpy=False)

Add function save_aligned_mesh: save axis-aligned scene into [scene_id]_axis_aligned.ply
Usage example:
export_one_scan('scene0000_00', './scans/scene0000_00/scene0000_00', savetxt=False, savenpy=False, save_aligned_mesh=True)
"""
...
SCANNET_DIR = 'scans'
SCAN_NAMES = sorted([line.rstrip() for line in open('meta_data/scannetv2_trainval.txt')]) #scannetv2.txt contains test scenes which could incur error
...
def export_one_scan(scan_name, output_filename_prefix, savetxt=False, savenpy=True, save_aligned_mesh=False):
    ...
    mesh_vertices, aligned_vertices, semantic_labels, instance_labels, instance_bboxes, aligned_instance_bboxes = export(mesh_file, agg_file, seg_file, meta_file, LABEL_MAP_FILE, None, save_aligned_mesh=save_aligned_mesh)
    ...
    if savetxt:
        txt_vert = np.concatenate((mesh_vertices, semantic_labels[:, np.newaxis], instance_labels[:, np.newaxis]),
                                  axis=1)
        aligned_txt_vert = np.concatenate(
            (aligned_vertices, semantic_labels[:, np.newaxis], instance_labels[:, np.newaxis]), axis=1)
        np.savetxt(output_filename_prefix + '_pts.txt', txt_vert, fmt="%s")
        np.savetxt(output_filename_prefix + '_aligned_pts.txt', aligned_txt_vert, fmt="%s")
    if savenpy:
