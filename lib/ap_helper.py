...
def parse_predictions(end_points, config_dict):
    ...
    """
    bsize, K, _ = end_points['center'].shape
    pred_corners_3d_upright_camera = end_points['bbox_corner'].detach().cpu().numpy()
    pred_sem_cls = end_points['sem_cls'] # B,num_proposal
    sem_cls_probs = softmax(end_points['sem_cls_scores'].detach().cpu().numpy()) # B,num_proposal,18
    pred_center = end_points['center'] # B,num_proposal,3 from proposal module

    # 1. remove predicted box whose #points < 5 like votenet
    nonempty_box_mask = np.ones((bsize, K))
    if config_dict['remove_empty_box']:  # True
        # -------------------------------------
    ...
    # for those non-empty predicted box, use nms to suppress overlapping
    ...
    if not config_dict['use_3d_nms']:
        ...
        end_points['pred_mask'] = pred_mask
        # ---------- NMS output: pred_mask in (B,K) -----------
    elif config_dict['use_3d_nms'] and config_dict['cls_nms']:  # True
        ...
        for i in range(bsize):
            ...
            pick = nms_3d_faster_samecls(boxes_3d_with_prob[nonempty_box_mask[i, :] == 1, :],
                                         config_dict['nms_iou'], config_dict['use_old_type_nms'])  # 0.25, False
            assert (len(pick) > 0)
            pred_mask[i, nonempty_box_inds[pick]] = 1
        end_points['pred_mask'] = pred_mask
        # ---------- NMS output: pred_mask in (B,K) -----------
    ...
    for i in range(bsize):
        if config_dict['per_class_proposal']:  # True
            cur_list = []
            for ii in range(config_dict['dataset_config'].num_class):
                cur_list += [(ii, pred_corners_3d_upright_camera[i, j], sem_cls_probs[i, j, ii] * obj_prob[i, j]) \
                             ...
                             pred_mask[i, j] == 1 and obj_prob[i, j] > config_dict['conf_thresh']]  # 0.05
...
def parse_groundtruths(end_points, config_dict, save_time=False):
    ...
    gt_corners_3d_upright_camera = end_points['gt_box_corner_label'].detach().cpu().numpy()
