...
class ProjectionHelper():
    ...
    def compute_projection(self, points, depth, camera_to_world):
        ...
        valid_ind_mask = torch.ge(image[0], 0) * torch.ge(image[1], 0) * torch.lt(image[0],
                                                                                  self.image_dims[0]) * torch.lt(
        ...
        depth_mask = depth_vals.ge(self.depth_min) * depth_vals.le(self.depth_max) * torch.abs(
            depth_vals - camera[2][valid_ind_mask]).le(self.accuracy)
        ...
        indices_3d = ind_update.new(num_points + 1).fill_(
            0)  # needs to be same size for all in batch... (first element has size)
        ...
        indices_2d[1:1 + indices_2d[0]] = torch.index_select(valid_image_ind, 0, torch.nonzero(depth_mask)[:,
                                                                                 0])  # indices of corresponding pixels
