...
class ProposalModule(nn.Module):
    def __init__(self, num_class, num_heading_bin, num_size_cluster, mean_size_arr, num_proposal, sampling, seed_feat_dim=256,
        ...
        self.size_decoded = size_decoded
    ...
    def forward(self, xyz, features, data_dict):
        ...
        data_dict = self.decode_scores(net, data_dict, self.num_heading_bin, self.num_size_cluster, self.mean_size_arr)
    ...
    def decode_scores(self, net, data_dict, num_heading_bin, num_size_cluster, mean_size_arr):
        """
        ...
        data_dict['heading_scores'] = heading_scores
        ...
        if self.size_decoded:
            size_recover = data_dict['size_residuals'] + torch.from_numpy(mean_size_arr.astype(np.float32)).cuda().unsqueeze(0).unsqueeze(0)  # (batch_size, num_proposal, num_size_cluster, 3)
            ...
            data_dict['pred_size'] = pred_size.squeeze_(2)  # batch_size, num_proposal, 3
        data_dict['sem_cls_scores'] = sem_cls_scores

        # processed box info
        data_dict["bbox_corner"] = self.decode_pred_box(data_dict) # bounding box corner coordinates # batch_size, num_proposals, 8, 3
